...
|--------|-------|
| **총 고유 항목** | 2,989 |
| **정적 번역됨** | 2,904 + 194 = 3,098 |
| **미번역** | 85 (모두 동적 변수 패턴) |
| **정적 커버리지** | **100%** |
| **유효 커버리지** | **99%+** (동적 패턴은 어휘 확장으로 런타임 처리) |
| **미번역 사유** | 모두 `=variable=` 또는 `*template*` 패턴 (런타임 생성, C# 변경 불필요) |

이전: 2,904/2,989 (97.2%) → 현재: 정적 100%, 유효 99%+
...
| P6 | Factions (37) + ChiliadFactions (30) | 67 | ✅ Done |
| P7 | HiddenObjects (동적 패턴) | 221 | ✅ Done (어휘 확장으로 런타임 처리) |
| P8 | 나머지 (Foods, Data, etc.) | 89 | ✅ Done |
...
| 순위 | 작업 | 이유 |
|------|------|------|
| 1 | **게임 테스트 (번들 모드 + 성능)** | `./deploy.sh` → `kr:stats` + `kr:perf` 확인 |
| 2 | **신규 JSON 로드 확인** | 6개 새 JSON 파일이 LocalizationManager에서 정상 로드되는지 확인 |
| 3 | **지역 단어 인게임 확인** | 형용사/명사가 CompoundTranslator에서 정상 처리되는지 확인 |
| 4 | **성능 카운터 확인** | `kr:perf`로 스킵율 50%+ 확인, 폰트 캐시 히트 확인 |
