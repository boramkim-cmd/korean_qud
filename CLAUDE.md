...
10. **배포 전 모드 에러 검사** — `./deploy.sh` 실행 전, 모드 내부 Harmony 패치 간 충돌(동일 메서드 중복 패치, 네임스페이스 오류, 컴파일 에러 등)이 없는지 검증. 게임 로그에서 에러/예외 확인 필수
11. **클래스/네임스페이스 추정 금지** — 게임 API의 클래스명, 네임스페이스, 메서드 시그니처를 추정하지 말 것. 반드시 게임 로그, 디컴파일된 소스, 또는 `.claude/source-findings.md` 등 확인된 출처에서 검증한 후 사용
